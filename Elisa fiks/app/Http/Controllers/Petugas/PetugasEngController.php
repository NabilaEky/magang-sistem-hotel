...
class PetugasEngController extends Controller
{
    ...
    public function profile()
    {
        ...
        $todayComplaint = Complaint::where('user_id', $user->id)
            ->where('status', 'selesai')
            ->whereDate('updated_at', now())
            ->count();

        $todayProker = LaporanEng::where('user_id', $user->id)
            ->whereDate('created_at', now())
            ->count();

        $todayCount = $todayComplaint + $todayProker;
    ...
    public function toggleNotif()
    {
        ...
        return back()->with('success', 'Notifikasi berhasil diubah');
    }
}
